...
export type WorkspaceRecord = { id: string; title: string; context: string; owner: string; state: OperatingState; amountPaise: number; source: string };
...
const stateFor = (value: string, index: number): OperatingState => ["HEALTHY", "PENDING", "RECONCILED", "OVERDUE", "REJECTED"][Math.abs([...value].reduce((sum, char) => sum + char.charCodeAt(0), index)) % 5] as OperatingState;
...
function sourceRecords(module: PlatformModuleDefinition, submodule: PlatformSubmoduleDefinition | undefined, snapshot: PlatformSnapshot): WorkspaceRecord[] {
  ...
  const assets = () => labelled("Asset and vendor registry", first(snapshot.assets, offset).map((item) => ({ id: item.id, title: `${item.oem} ${item.model}`, context: `${item.category} · ${item.serialNumber}`, owner: "vendor-operations", state: "HEALTHY", amountPaise: item.invoiceValuePaise })));
  ...
  const integrations = () => labelled("Integration adapter monitor", first(snapshot.integrations, offset).map((item) => ({ id: item.id, title: item.name, context: `${item.mode} · accepted ${item.accepted}`, owner: "platform-admin", state: item.status === "FAILED" ? "REJECTED" : item.status === "PARTIAL" ? "PENDING" : "HEALTHY", amountPaise: 0 })));
  const audits = () => labelled("Configuration and audit evidence", first(snapshot.auditEvents, offset).map((item, index) => ({ id: item.id, title: item.action.replaceAll("_", " "), context: `${item.entityType} · ${item.entityId}`, owner: item.actorId, state: stateFor(item.action, index), amountPaise: 0 })));
  const profiles = () => labelled("IAM profile configuration", first(snapshot.profiles, offset).map((item, index) => ({ id: item.userId, title: item.userId, context: item.roleKeys.join(", "), owner: "platform-admin", state: item.isAdmin ? "HEALTHY" : stateFor(item.userId, index), amountPaise: 0 })));
...
export function ModuleWorkspace({ module, submodule, snapshot, filters }: { module: PlatformModuleDefinition; submodule?: PlatformSubmoduleDefinition; snapshot: PlatformSnapshot; filters: RouteFilters }) {
  ...
  useEffect(() => { const onKey = (event: KeyboardEvent) => { if (event.key === "Escape" && selected) close(); }; window.addEventListener("keydown", onKey); return () => window.removeEventListener("keydown", onKey); }, [selected]);
