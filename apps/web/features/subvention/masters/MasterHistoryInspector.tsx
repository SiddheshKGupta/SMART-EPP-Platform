...
export function MasterHistoryInspector({
  ...
  const history = events
    .filter((event) => event.entityType === "MasterRecord" && event.entityId === record.id)
          ...
          <ol className="mt-3 border-l border-slate-300 pl-4">
            {history.map((event) => (
              <li className="relative pb-5 text-xs last:pb-0" key={event.id}>
                ...
                <div className="font-semibold text-slate-900">{event.action === "MASTER_DEACTIVATED" ? "Deactivated" : "Saved"}</div>
