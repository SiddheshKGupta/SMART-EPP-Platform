...
export type ExceptionScenario = "RETURNED" | "DUPLICATED" | "MISMATCHED";
export type ExceptionRecoveryState = "OPEN" | "IN_PROGRESS" | "RECOVERED";
export type AuditOutcome = "SUCCESS" | "DENIED" | "NO_CHANGE" | "PARTIAL" | "FAILURE";
...
export interface EmployeeRecord { id: string; employerId: string; name: string; payrollId: string; pan: string; bankAccount: string; status: OperatingState; }
...
export interface WorkItem { id: string; employerId: string; module: PlatformModuleKey; title: string; owner: string; assignedUserId?: string; queueKeys: WorkQueueKey[]; initiatedBy?: string; requestedAction?: PlatformAction; dueDate: string; state: OperatingState; financialImpactPaise: number; href: string; completedAt?: string; completedBy?: string; }
...
export interface PlatformAuditEvent { id: string; entityType: string; entityId: string; action: string; actorId: string; reason: string; outcome: AuditOutcome; occurredAt: string; }
export interface PlatformExceptionRecord { id: string; employerId: string; scenario: ExceptionScenario; operatingState: OperatingState; recoveryState: ExceptionRecoveryState; sourceRecordType: "Application" | "Employee" | "IntegrationAdapter"; sourceRecordId: string; workItemId: string; }
